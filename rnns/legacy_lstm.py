...
class LSTM(Cell):
    ...
    def __init__(self,
        ...
        if self.is_ascend:
            ...
            print(hidden_size, num_layers, gate_size)
            for layer in range(num_layers):
...
if __name__ == '__main__':
    print('start')
    net = LSTM(10, 20, 2, has_bias=True, batch_first=False, bidirectional=False)
    x = Tensor(np.ones([1, 2, 10]).astype(np.float32))
    h0 = Tensor(np.ones([1 * 2, 2, 20]).astype(np.float32))
    c0 = Tensor(np.ones([1 * 2, 2, 20]).astype(np.float32))
    output, (hn, cn) = net(x, (h0, c0))
    print(output.shape)